...
public class Main {
	...
	public static void mostarNombreSedeConMasProyectos() {
		
		Session sesion = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = sesion.beginTransaction();
        
        String hql = "SELECT ps.sede.nombre, COUNT(ps.proyecto.id) " +
                "FROM ProyectoSede ps " +
                "GROUP BY ps.sede.nombre " +
                "ORDER BY COUNT(ps.proyecto.id) DESC";
	}
